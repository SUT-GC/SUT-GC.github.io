...
|---|---|---|
| ① 入口层 | CLI、TUI、Web UI、macOS App、iOS、Android | 不做。飞书就是我的入口 |
| ② 消息渠道层 | WhatsApp（逆向协议）、Telegram、Discord、Slack 等 10+ | 只做飞书。官方 SDK，WebSocket 长连接 |
| ③ Gateway 层 | 消息路由、会话管理、状态机、健康监控、Boot 机制、执行审批 | **完整实现**。这是心脏 |
| ④ Hooks & Cron 层 | 事件总线、cron 定时任务 | 做 cron，hooks 暂时不需要 |
| ⑤ Agent 执行层 | Claude Code CLI / Codex CLI，双引擎，嵌入式运行 | **完整实现——但抽成了独立框架** |
...
所以我干了一件事：**把这层抽出来，做成了一个独立框架** [Cody](https://github.com/CodyCodeAgent/cody)。一句话说清楚它的定位：让你用几行代码就拿到一个带工具、带流式、带上下文管理的 Agent 运行时。它怎么设计的我后面会专门写一篇。

为什么要跟你说这个？因为如果你也想造自己的 Agent 应用，**最大的建议就是：不要从零写执行层**。不管用 Cody 还是 LangChain 还是别的什么，找一个趁手的框架把这层接住，把精力留给真正有意思的部分——Gateway 和业务逻辑。
...
你跟 ChatGPT 聊天的时候，发完一句它会记得你前面说了什么，这就是"会话"。但如果你关掉页面明天再来，它会开一个新的对话——旧的上下文不会自动带过来。
...
> 如果你也用 Docker 部署，**记得给 SQLite 文件挂 volume**，否则容器重建数据就没了。
