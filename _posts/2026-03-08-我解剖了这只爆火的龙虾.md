...
layout: post
title: "我解剖了这只爆火的龙虾"
description: "OpenClaw 两个月 14 万 Star，我把源码读了一遍，搞清楚了它的八层架构——以及怎么用现有开源工具自己搭一个。"
categories: ["技术"]
...
最近 OpenClaw（前身 Clawdbot）火遍硅谷到北京，有人说它是"AI 助手的终局形态"。一个开源项目，两个月内斩获 **14 万+ GitHub Stars**，让一堆人去买 Mac Mini 专门跑它，导致Mac Mini二手市场价格暴涨。

当你在使用它的同时，有没有想过，它是怎么运作的？作为程序员的我，把整个仓库 clone 下来，结合AI的帮助，读了一遍源码。4700+ 个 TypeScript/JavaScript 文件，38MB 源码目录，800+ 个 agent 相关模块。这篇文章就是我的读后感——你可以跟着我一起**从技术视角，搞清楚它到底是怎么做到的**。

我先说结论是：其实没那么玄。

为了避免篇幅过长，关于OpenClaw，我会写两篇文章，第一篇专讲OpenClaw的架构链路，第二篇，我们会一起来实现一个简单的OpenClaw。
...
它不是聊天机器人，不是 Zapier，也不是套壳 AI。核心差异在下面三个词：
> Zapier 是一个无代码自动化平台
...
<div style="max-width: 600px; margin: 24px auto; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; font-size: 14px;">
  <div style="background: linear-gradient(135deg, #4a5568, #2d3748); color: #fff; padding: 14px 20px; border-radius: 12px 12px 0 0; text-align: center;">
    <strong>第一层：入口层</strong><br><span style="font-size: 12px; opacity: 0.85;">macOS / iOS / Android / Web UI / CLI / TUI</span>
  </div>
  <div style="background: linear-gradient(135deg, #667eea, #764ba2); color: #fff; padding: 14px 20px; text-align: center;">
    <strong>第二层：消息渠道层</strong><br><span style="font-size: 12px; opacity: 0.85;">WhatsApp / Telegram / Discord / iMessage...</span>
  </div>
  <div style="background: linear-gradient(135deg, #764ba2, #e96443); color: #fff; padding: 14px 20px; text-align: center;">
    <strong>第三层：Gateway 核心层</strong><br><span style="font-size: 12px; opacity: 0.85;">路由、认证、会话管理、WebSocket</span>
  </div>
  <div style="background: linear-gradient(135deg, #e96443, #e05d3b); color: #fff; padding: 14px 20px; text-align: center;">
    <strong>第四层：Hooks / 自动化层</strong><br><span style="font-size: 12px; opacity: 0.85;">事件钩子、Cron 定时任务、自动回复</span>
  </div>
  <div style="background: linear-gradient(135deg, #e05d3b, #f5af19); color: #fff; padding: 14px 20px; text-align: center;">
    <strong>第五层：Agent 执行层</strong><br><span style="font-size: 12px; opacity: 0.85;">CLI 后端、权限模型、ACP 多智能体</span>
  </div>
  <div style="background: linear-gradient(135deg, #f5af19, #43b89c); color: #fff; padding: 14px 20px; text-align: center;">
    <strong>第六层：LLM Provider &nbsp;·&nbsp; 浏览器 & 媒体</strong><br><span style="font-size: 12px; opacity: 0.85;">28+ 提供商、failover · CDP、截图、自动化</span>
  </div>
  <div style="background: linear-gradient(135deg, #43b89c, #3a7bd5); color: #fff; padding: 14px 20px; text-align: center;">
    <strong>第七层：记忆系统 &nbsp;·&nbsp; 技能/插件</strong><br><span style="font-size: 12px; opacity: 0.85;">SQLite + 向量 + FTS · SKILL.md + Plugin SDK</span>
  </div>
  <div style="background: linear-gradient(135deg, #3a7bd5, #2d3748); color: #fff; padding: 14px 20px; border-radius: 0 0 12px 12px; text-align: center;">
    <strong>第八层：基础设施 & 配置</strong><br><span style="font-size: 12px; opacity: 0.85;">Config schema、TLS、设备发现、日志、安全</span>
  </div>
</div>
...
<div style="max-width: 580px; margin: 24px auto; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; font-size: 13px;">
  <div style="background: #2d3748; color: #e2e8f0; padding: 12px 16px; border-radius: 10px 10px 0 0; text-align: center; font-weight: 600;">
    Gateway (WebSocket Server · ws://localhost:18789)
  </div>
  <div style="display: grid; grid-template-columns: 1fr 1fr; border-left: 2px solid #4a5568; border-right: 2px solid #4a5568;">
    <div style="background: #667eea; color: #fff; padding: 10px 12px; text-align: center; border-right: 1px solid rgba(255,255,255,0.2);">
      <strong>消息路由</strong><br><span style="font-size: 11px; opacity: 0.85;">状态机 · 心跳 · 分发</span>
    </div>
    <div style="background: #764ba2; color: #fff; padding: 10px 12px; text-align: center;">
      <strong>会话管理</strong><br><span style="font-size: 11px; opacity: 0.85;">JSONL · 上下文 · 历史</span>
    </div>
    <div style="background: #e96443; color: #fff; padding: 10px 12px; text-align: center; border-right: 1px solid rgba(255,255,255,0.2);">
      <strong>渠道健康监控</strong><br><span style="font-size: 11px; opacity: 0.85;">5 分钟巡检 · 自动重连</span>
    </div>
    <div style="background: #e05d3b; color: #fff; padding: 10px 12px; text-align: center;">
      <strong>配置热重载</strong><br><span style="font-size: 11px; opacity: 0.85;">chokidar 监听 · 300ms 防抖</span>
    </div>
    <div style="background: #43b89c; color: #fff; padding: 10px 12px; text-align: center; border-right: 1px solid rgba(255,255,255,0.2);">
      <strong>设备配对</strong><br><span style="font-size: 11px; opacity: 0.85;">Bonjour · token · 审批</span>
    </div>
    <div style="background: #3a7bd5; color: #fff; padding: 10px 12px; text-align: center;">
      <strong>Boot 启动脚本</strong><br><span style="font-size: 11px; opacity: 0.85;">BOOT.md · 自然语言启动</span>
    </div>
    <div style="background: #f5af19; color: #fff; padding: 10px 12px; text-align: center; border-right: 1px solid rgba(255,255,255,0.2);">
      <strong>接入认证</strong><br><span style="font-size: 11px; opacity: 0.85;">token · 密码 · Tailscale</span>
    </div>
    <div style="background: #d69e2e; color: #fff; padding: 10px 12px; text-align: center;">
      <strong>执行审批</strong><br><span style="font-size: 11px; opacity: 0.85;">命令白名单 · 远程确认</span>
    </div>
  </div>
  <div style="background: #4a5568; color: #a0aec0; padding: 8px 16px; border-radius: 0 0 10px 10px; text-align: center; font-size: 11px;">
    100+ API 方法：channels.* · sessions.* · cron.* · config.* · node.* · agents.* · ...
  </div>
</div>
...
```
用户消息 → Agent 要执行命令
  → OpenClaw 执行审批层（src/infra/exec-approvals.ts）
    ├─ 命令在白名单 → 放行
    ├─ 不在白名单 + ask=on-miss → 推送到用户渠道，等 120 秒确认
    └─ 超时 / 用户拒绝 → 拒绝执行
  → 放行后 → 调用 claude --permission-mode bypassPermissions
    → Claude Code 直接执行，不再二次确认
```
...
用户提问
  → 同时发起两路搜索：
    ├─ 向量搜索：把提问嵌入为向量，计算余弦相似度（权重 70%）
    └─ BM25 全文搜索：提取关键词，在 FTS5 表上检索（权重 30%）
  → 合并两路结果，加权评分
  → 可选：时间衰减（越旧的记忆分数越低，半衰期 30 天）
  → 可选：MMR 重排序（兼顾相关性和多样性）
  → 返回 top-6 结果
```
